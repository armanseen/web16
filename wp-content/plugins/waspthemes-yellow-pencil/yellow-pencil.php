...
function yp_advanced_link_replace($match){
    ...
    $linkHref = yp_unparse_url($linkHrefParsed);
...
function yp_link_replace($buffer){
    ...
    // Replace links
    $buffer = preg_replace_callback('@\<link([^\>]+)href=(\s+)?(\"|\')(.*?)(\s+)?(\"|\')([^\>]+)@', "yp_advanced_link_replace", $buffer);
...
define('YP_FUNCTION_PATH_FOR_CUSTOM_CHANGES',plugin_basename(__FILE__));
